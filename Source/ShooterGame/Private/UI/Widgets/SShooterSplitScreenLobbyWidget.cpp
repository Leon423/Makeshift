...
void SShooterSplitScreenLobby::Construct( const FArguments& InArgs )
{
	...
	FString Msg = LOCTEXT("You must be signed in to play online", "You must be signed in to play online").ToString();
	...
	if(OnlineSub)
	{
		...
		if(IdentityInterface.IsValid())
		{
			...
			for ( int i = 0; i < NumActiveSlots; ++i )
			{
				...
				TSharedPtr<FUniqueNetId> Sponsor = IdentityInterface->GetSponsorUniquePlayerId( LocalPlayer->ControllerId );
			...
			if (Identity.IsValid())
			{
				...
				if (ShooterViewport != NULL)
				{
					ShooterViewport->ShowDialog(
						...
						Nickname + FString(" cannot play due to age restrictions!"),
						FString("OK"),
						FString(),
				...
				else
				{
					...
					if ( ShooterViewport != NULL )
					{
						ShooterViewport->ShowDialog( 
							PlayerOwner.Get(),
							EShooterDialogType::Generic,
							FString("A guest account cannot play without its sponsor!"),
							FString("OK"),
							FString(),
							FOnClicked::CreateRaw(this, &SShooterSplitScreenLobby::OnOkOrCancel),
							FOnClicked::CreateRaw(this, &SShooterSplitScreenLobby::OnOkOrCancel)
