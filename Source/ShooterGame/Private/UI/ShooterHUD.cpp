...
AShooterHUD::AShooterHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	ShieldBar = UCanvas::MakeIcon(HUDAssets02Texture, 67, 212, 372, 50);
	ShieldBarBg = UCanvas::MakeIcon(HUDAssets02Texture, 67, 162, 372, 50);
	ShieldIcon = UCanvas::MakeIcon(HUDAssets02Texture, 78, 262, 28, 28);
}
...
void AShooterHUD::PostInitializeComponents()
{
	...
	if (OnlineSub)
	{
		...
		if (Voice.IsValid())
		{
			Voice->AddOnPlayerTalkingStateChangedDelegate(OnPlayerTalkingStateChangedDelegate);
		}
	}
}
...
// ADDED THIS
void AShooterHUD::DrawShield()
{
	AShooterCharacter* MyPawn = Cast<AShooterCharacter>(GetOwningPawn());
	Canvas->SetDrawColor(FColor::White);
	const float ShieldPosX = (Canvas->ClipX - ShieldBarBg.UL * ScaleUI) / 2;
	const float ShieldPosY = Canvas->ClipY - (Offset + ShieldBarBg.VL) - 100 * ScaleUI;
	Canvas->DrawIcon(ShieldBarBg, ShieldPosX, ShieldPosY, ScaleUI);
	const float ShieldAmount = FMath::Min(1.0f, MyPawn->CurrentShieldAmount / MyPawn->MaxShieldAmount);

	FCanvasTileItem TileItem(FVector2D(ShieldPosX, ShieldPosY), ShieldBar.Texture->Resource,
		FVector2D(ShieldBar.UL * ShieldAmount * ScaleUI, ShieldBar.VL * ScaleUI), FLinearColor::White);
	MakeUV(ShieldBar, TileItem.UV0, TileItem.UV1, ShieldBar.U, ShieldBar.V, ShieldBar.UL * ShieldAmount, ShieldBar.VL);
	TileItem.BlendMode = SE_BLEND_Translucent;
	Canvas->DrawItem(TileItem);

	Canvas->DrawIcon(ShieldIcon, ShieldPosX + Offset * ScaleUI, ShieldPosY + (ShieldBar.VL - ShieldIcon.VL) / 2.0f * ScaleUI, ScaleUI);
}
