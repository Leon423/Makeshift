...
void FShooterWelcomeMenu::HandleLoginUIClosed(TSharedPtr<FUniqueNetId> UniqueId, const int ControllerIndex)
{
	...
	{
		const FString StopReason	= NSLOCTEXT( "ProfileMessages", "NeedLicense", "The signed in users do not have a license for this game. Please purchase ShooterGame from the Xbox Marketplace or sign in a user with a valid license." ).ToString();
...
void FShooterWelcomeMenu::OnUserCanPlay(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{
	...
	else
	{
		...
		if ( ShooterViewport != NULL )
		{
			const FString ReturnReason = NSLOCTEXT("PrivilegeFailures", "CannotPlayAgeRestriction", "You cannot play this game due to age restrictions.").ToString();
			const FString OKButton = NSLOCTEXT("DialogButtons", "OKAY", "OK").ToString();
