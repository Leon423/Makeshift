...
void UShooterGameInstance::Init()
{
	...
	{
		IdentityInterface->AddOnLoginStatusChangedDelegate(i, FOnLoginStatusChangedDelegate::CreateUObject(this, &UShooterGameInstance::HandleUserLoginChanged));
	}

	IdentityInterface->AddOnControllerPairingChangedDelegate(FOnControllerPairingChangedDelegate::CreateUObject(this, &UShooterGameInstance::HandleControllerPairingChanged));
	...
	OnlineSub->AddOnConnectionStatusChangedDelegate( FOnConnectionStatusChangedDelegate::CreateUObject( this, &UShooterGameInstance::HandleNetworkConnectionStatusChanged ) );

	OnEndSessionCompleteDelegate = FOnEndSessionCompleteDelegate::CreateUObject(this, &UShooterGameInstance::OnEndSessionComplete);

	SessionInterface->AddOnSessionUserInviteAcceptedDelegate( FOnSessionUserInviteAcceptedDelegate::CreateUObject( this, &UShooterGameInstance::HandleSessionUserInviteAccepted ) );
...
#if PLATFORM_XBOXONE
		const FString ReturnReason	= NSLOCTEXT( "NetworkFailures", "ServiceUnavailable", "Connection to Xbox LIVE has been lost." ).ToString();
#elif PLATFORM_PS4
		const FString ReturnReason	= NSLOCTEXT( "NetworkFailures", "ServiceUnavailable", "Connection to \"PSN\" has been lost." ).ToString();
#else
		const FString ReturnReason	= NSLOCTEXT( "NetworkFailures", "ServiceUnavailable", "Connection has been lost." ).ToString();
#endif
		const FString OKButton		= NSLOCTEXT( "DialogButtons", "OKAY", "OK" ).ToString();
		
		ShowMessageThenGotoState( ReturnReason, OKButton, FString(), ShooterGameInstanceState::MainMenu );
	}
...
void UShooterGameInstance::ShowMessageThenGotoState( const FString& Message, const FString& OKButtonString, const FString& CancelButtonString, const FName& NewState, const bool OverrideExisting, TWeakObjectPtr< ULocalPlayer > PlayerOwner )
{
...
void UShooterGameInstance::ShowMessageThenGoMain(const FString& Message, const FString& OKButtonString, const FString& CancelButtonString)
{
...
void UShooterGameInstance::RemoveNetworkFailureHandlers()
{
	...
	{
		GEngine->OnTravelFailure().RemoveUObject(this, &UShooterGameInstance::TravelLocalSessionFailure);
	}
}
...
void UShooterGameInstance::AddNetworkFailureHandlers()
{
	...
	{
		GEngine->OnTravelFailure().AddUObject(this, &UShooterGameInstance::TravelLocalSessionFailure);
	}
}
...
bool UShooterGameInstance::JoinSession(ULocalPlayer* LocalPlayer, int32 SessionIndexInSearchResults)
{
	...
	if (GameSession)
	{
		...
		GameSession->OnJoinSessionComplete().AddUObject(this, &UShooterGameInstance::OnJoinSessionComplete);
...
void UShooterGameInstance::OnJoinSessionComplete(EOnJoinSessionCompleteResult::Type Result)
{
	...
	if (GameSession)
	{
		GameSession->OnJoinSessionComplete().RemoveUObject(this, &UShooterGameInstance::OnJoinSessionComplete);
	}
...
void UShooterGameInstance::OnCreatePresenceSessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (GameSession)
	{
		GameSession->OnCreatePresenceSessionComplete().RemoveUObject(this, &UShooterGameInstance::OnCreatePresenceSessionComplete);
...
bool UShooterGameInstance::FindSessions(ULocalPlayer* PlayerOwner, bool bFindLAN)
{
	...
	if (PlayerOwner)
	{
		...
		if (GameSession)
		{
			...
			GameSession->OnFindSessionsComplete().AddUObject(this, &UShooterGameInstance::OnSearchSessionsComplete);
...
void UShooterGameInstance::OnSearchSessionsComplete(bool bWasSuccessful)
{
	...
	if (Session)
	{
		Session->OnFindSessionsComplete().RemoveUObject(this, &UShooterGameInstance::OnSearchSessionsComplete);
	}
}
...
bool UShooterGameInstance::Tick(float DeltaSeconds)
{
	...
	if (CurrentState != ShooterGameInstanceState::WelcomeScreen)
	{
		...
		{
			const FString ReturnReason	= NSLOCTEXT( "ProfileMessages", "NeedLicense", "The signed in users do not have a license for this game. Please purchase ShooterGame from the Xbox Marketplace or sign in a user with a valid license." ).ToString();
			const FString OKButton		= NSLOCTEXT( "DialogButtons", "OKAY", "OK" ).ToString();

			ShowMessageThenGotoState( ReturnReason, OKButton, FString(), ShooterGameInstanceState::WelcomeScreen );
		}
			...
			!ShooterViewport->IsShowingDialog())
		{
			for (int i = 0; i < LocalPlayers.Num(); ++i)
			{
				if (LocalPlayers[i] && LocalPlayers[i]->ControllerId == -1)
				{
					ShooterViewport->ShowDialog( 
						LocalPlayers[i],
						EShooterDialogType::ControllerDisconnected,
						FString::Printf(TEXT("Player %d, please reconnect your controller."), i + 1),
#ifdef PLATFORM_PS4
					FString("Cross Button - Continue"),
#else
					FString("A - Continue"),
#endif
						FString(),
...
#if SHOOTER_CONSOLE_UI
		// Display message on consoles
		const FString ReturnReason	= NSLOCTEXT( "ProfileMessages", "SignInChange", "Sign in status change occurred." ).ToString();
		const FString OKButton		= NSLOCTEXT( "DialogButtons", "OKAY", "OK" ).ToString();

		ShowMessageThenGotoState(ReturnReason, OKButton, FString(), GetInitialState());
#else								
		GotoInitialState();
#endif
	}
...
void UShooterGameInstance::HandleUserLoginChanged(int32 GameUserIndex, ELoginStatus::Type PreviousLoginStatus, ELoginStatus::Type LoginStatus, const FUniqueNetId& UserId)
{
	...
	//a better check would be to only be 'upgraded' if we're in the menu system and change, but that might miss a few corner cases.
	//whole thing will be moot when we upgrade the netid system to work by handles.
	const bool bUpgraded = LoginStatus == ELoginStatus::LoggedIn;
	...
	if (bUpgraded)
	{
		...
		// If this user is now logged in for multiplayer but previously wasn't then we need to bail because any NetId's that the game may have cached are now invalid.
		// (Cached netids for offline players may not have platform specific data needed for online OSS calls).
		// In the future we want NetIds to be handles to avoid this.
		if (LocalPlayer != NULL)
		{
			HandleSignInChangeMessaging();
		}
	...
	if ( ControlledLocalPlayer != nullptr && ControlledLocalPlayer != NewLocalPlayer )
	{
		...
		if ( ShooterViewport != nullptr )
		{
			ShooterViewport->ShowDialog( 
				...
				NSLOCTEXT("ProfileMessages", "PairingChanged", "Your controller has been paired to another profile, would you like to switch to this new profile now? Selecting YES will sign out of the previous profile.").ToString(),
				NSLOCTEXT("DialogButtons", "YES", "A - YES").ToString(),
				NSLOCTEXT("DialogButtons", "NO", "B - NO").ToString(),
...
bool UShooterGameInstance::ValidatePlayerForOnlinePlay(ULocalPlayer* LocalPlayer)
{
	...
	if (CurrentConnectionStatus != EOnlineServerConnectionStatus::Connected)
	{
		...
		if (ShooterViewport != NULL)
		{
			const FString Msg				= NSLOCTEXT("NetworkFailures", "ServiceDisconnected", "You must be connected to the Xbox LIVE service to play online.").ToString();
			const FString OKButtonString	= NSLOCTEXT("DialogButtons", "OKAY", "OK").ToString();
	...
	if (!IsLocalPlayerOnline(LocalPlayer))
	{
		...
		if (ShooterViewport != NULL)
		{
			const FString Msg				= NSLOCTEXT("NetworkFailures", "MustBeSignedIn", "You must be signed in to play online").ToString();
			const FString OKButtonString	= NSLOCTEXT("DialogButtons", "OKAY", "OK").ToString();
...
void UShooterGameInstance::StartOnlinePrivilegeTask(const IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate& Delegate, EUserPrivileges::Type Privilege, TSharedPtr< FUniqueNetId > UserId)
{
	WaitMessageWidget = SNew(SShooterWaitDialog)
		.MessageText(TEXT("Checking privileges with server.  Please wait..."));
...
{	
	...
	if (ShooterViewport != NULL && OwningPlayer.IsValid())
	{
		if ((PrivilegeResults & (uint32)IOnlineIdentity::EPrivilegeResults::AccountTypeFailure) != 0)
		{
			...
			if (OnlineSub)
			{
				OnlineSub->DisplayAccountUpgradeDialog(UserId);
			}
		}
		else if ((PrivilegeResults & (uint32)IOnlineIdentity::EPrivilegeResults::RequiredSystemUpdate) != 0)
		{
			ShooterViewport->ShowDialog(
				OwningPlayer.Get(),
				EShooterDialogType::Generic,
				FString("A required system update is available.  Please upgrade to access online features."),
				FString("OK"),
				FString(),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric)
		...
		else if ((PrivilegeResults & (uint32)IOnlineIdentity::EPrivilegeResults::RequiredPatchAvailable) != 0)
		{
			ShooterViewport->ShowDialog(
				OwningPlayer.Get(),
				EShooterDialogType::Generic,
				FString("A required game patch is available.  Please upgrade to access online features."),
				FString("OK"),
				FString(),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric)
		...
		else if ((PrivilegeResults & (uint32)IOnlineIdentity::EPrivilegeResults::AgeRestrictionFailure) != 0)
		{
			ShooterViewport->ShowDialog(
				OwningPlayer.Get(),
				EShooterDialogType::Generic,
				FString("Cannot play due to age restrictions!"),
				FString("OK"),
				FString(),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric)
		...
		else if ((PrivilegeResults & (uint32)IOnlineIdentity::EPrivilegeResults::UserNotFound) != 0)
		{
			ShooterViewport->ShowDialog(
				OwningPlayer.Get(),
				EShooterDialogType::Generic,
				FString("Cannot play due invalid user!"),
				FString("OK"),
				FString(),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric)
		...
		else if ((PrivilegeResults & (uint32)IOnlineIdentity::EPrivilegeResults::GenericFailure) != 0)
		{
			ShooterViewport->ShowDialog(
				OwningPlayer.Get(),
				EShooterDialogType::Generic,
				FString("Cannot play online.  Check your network connection."),
				FString("OK"),
				FString(),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric),
				FOnClicked::CreateUObject(this, &UShooterGameInstance::OnConfirmGeneric)
