...
void AShooterPlayerController::TickActor(float DeltaTime, enum ELevelTick TickType, FActorTickFunction& ThisTickFunction)
{
	...
	if (IsGameMenuVisible())
	{
		...
		else
		{
			...
			if (ShooterFriends.IsValid() && LocalPlayer && LocalPlayer->ControllerId >= 0)
			{
				ShooterFriends->UpdateFriends(LocalPlayer->ControllerId);
			}
...
void AShooterPlayerController::QueryAchievements()
{
	...
	ULocalPlayer* LocalPlayer = Cast<ULocalPlayer>(Player);
	if (LocalPlayer && LocalPlayer->ControllerId != -1)
	{
...
{		
	...
	{
		const FString ReturnReason	= NSLOCTEXT( "NetworkErrors", "HostQuit", "The host has quit the match." ).ToString();
		const FString OKButton		= NSLOCTEXT( "DialogButtons", "OKAY", "OK" ).ToString();

		SGI->ShowMessageThenGotoState( ReturnReason, OKButton, FString(), ShooterGameInstanceState::MainMenu );
	}
	else
	{
